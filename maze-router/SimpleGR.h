...
class PQueue
{
  ...
  public:
    ...
    {
      cout << "setBits[] = " << "\n";
      for (unsigned b = 0; b < setBits.size(); b++)
      {
        cout << b << ": " << setBits[b];
        if (b != setBits.size()-1)
          cout << "\n";
      ...
      cout << "\n";
      cout << "bits[] = " << "\n";
      for (unsigned c = 0; c < bits.size(); c++)
      {
        cout << c << ": ";
        if (bits[c])
          cout << "true";
...
public:
  ...
  // Displays gcell data for all gcells
  void showGCellData()
  {
    cout << "GCellData[] = " << "\n";
    for (unsigned d = 0; d < data.size(); d++)
    {
      cout << "  " << d << ": heapLoc = " << data[d].heapLoc << " totalCost = " << data[d].totalCost << " pathCost = " << data[d].pathCost << " parentGCell = " << data[d].parentGCell << "\n";
    }
    dataValid.show();
  }
...
private:
  ...
  // @brief: get the neighboring gcells from a gcell within bounds
  vector<Point> getGCellPoints(const Point gcell, const Point botleft, const Point topright) {
    Point gcellTemp;
    gcellTemp = gcell;
    vector<Point> GCellPoints;
    // only check for horizontal neighbors if gcell layer allows horizontal edges
    if (horizCaps[gcell.z] != 0)
    {
      int minX = botleft.x;
      int maxX = topright.x;
      // check if gcell has a neighbor to it's left
      if (gcell.x-1 >= minX) {
        gcellTemp.x = gcell.x-1;
        GCellPoints.push_back(gcellTemp);
        gcellTemp.x++;
      }
      // check if gcell has a neighbor to it's right
      if (gcell.x+1 < maxX) {
        gcellTemp.x = gcell.x+1;
        GCellPoints.push_back(gcellTemp);
        gcellTemp.x--;
    ...
    // only check for vertical neighbors if gcell layer allows vertical edges
    if (vertCaps[gcell.z] != 0)
    {
      int minY = botleft.y;
      int maxY = topright.y;
      // check if gcell has a neighbor to it's down
      if (gcell.y-1 >= minY) {
        gcellTemp.y = gcell.y-1;
        GCellPoints.push_back(gcellTemp);
        gcellTemp.y++;
      }
      // check if gcell has a neighbor to it's up
      if (gcell.y+1 < maxY) {
        gcellTemp.y = gcell.y+1;
        GCellPoints.push_back(gcellTemp);
        gcellTemp.y--;
    ...
    int maxZ = numLayers;
    // check if gcell has a neighbor to it's below
    if (gcell.z-1 >= 0) {
      gcellTemp.z = gcell.z-1;
      GCellPoints.push_back(gcellTemp);
      gcellTemp.z++;
    }
    // check if gcell has a neighbor to it's above
    if (gcell.z+1 < maxZ) {
      gcellTemp.z = gcell.z+1;
      GCellPoints.push_back(gcellTemp);
      gcellTemp.z--;
  ...
  // @brief: get the direction between two adjacent cells
  EdgeType getGCellsAdj(const Point a, const Point b) {
    // check if the two cells are left-right adjacent
    if (b.x == a.x-1 || b.x == a.x+1)
      return HORIZ;
    // check if the two cells are down-up adjacent
    if (b.y == a.y-1 || b.y == a.y+1)
      return VERT;
    // check if the two cells are below-above adjacent
    if (b.z == a.z-1 || b.z == a.z+1)
      return VIA;
  }

  // @brief: get the edge ID between two adjacent gcells
  IdType getEdgeId(const GCell a, const GCell b, EdgeType adj) {
    if (adj == HORIZ) {
      // Left-Right Edge
      if (a.incX == b.decX && a.incX != NULLID)
        return a.incX;
      if (a.decX == b.incX && a.decX != NULLID)
        return a.decX;
    }
    if (adj == VERT) {
      // Down-Up Edge
      if (a.incY == b.decY && a.incY != NULLID)
        return a.incY;
      if (a.decY == b.incY && a.decY != NULLID)
        return a.decY;
    }
    if (adj == VIA) {
      // Below-Above Edge
      if (a.incZ == b.decZ && a.incZ != NULLID)
        return a.incZ;
      if (a.decZ == b.incZ && a.decZ != NULLID)
        return a.decZ;
    }
    return NULLID;
  }

  // @brief: get the edge from the edge ID
  Edge* getEdge(const IdType edgeId) {
    for (unsigned n = 0; n < grEdgeArr.size(); ++n) {
      if (grEdgeArr[n].id == edgeId) {
        return &grEdgeArr[n];
      }
    }
  }

  // @brief: reset all GCellData in the priorityQueue
  void resetData() {
    for (unsigned n = 0; n < (gcellArrSzX * gcellArrSzY * numLayers); ++n)
      priorityQueue.setGCellCost(n, 0, 0, 0);
  }
